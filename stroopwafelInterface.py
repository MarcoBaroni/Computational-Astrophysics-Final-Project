...
num_systems = 1000000                 # Number of binary systems to evolve                                              # Note: overrides pythonSubmit value
...
R_SOL_TO_AU = constants.R_SOL_TO_AU
...
def create_dimensions():
    ...
    """
    m1 = classes.Dimension('--initial-mass-1', 5, 149.9, sampler.uniform, prior.uniform)
    q = classes.Dimension('q', 0.01, 1, sampler.uniform, prior.uniform, should_print = False)
    a = classes.Dimension('--semi-major-axis', .01*R_SOL_TO_AU, 10000*R_SOL_TO_AU, sampler.flat_in_log, prior.flat_in_log) 
    z = classes.Dimension('--metallicity', 10**(-4), 0.02, sampler.uniform, prior.uniform)
    e = classes.Dimension('--eccentricity', 0.0, 1., sampler.uniform, prior.uniform)
    return [m1,q,a,z,e]
...
def interesting_systems(batch):
    ...
    try:
        ...
        for index, sample in enumerate(batch['samples']): 
            ...
            sample.properties['time_common_enveloe_1'] = 0
            sample.properties['time_common_enveloe_2'] = 0
            sample.properties['bbh'] = 0
        ...
        all_bbh = double_compact_objects[np.logical_and(double_compact_objects['Stellar_Type(1)'] == 14, double_compact_objects['Stellar_Type(2)'] == 14)]  # sistemi di BBH, serve per capire quali sistemi diventano BBH senza collassare
        all_bbh_seeds = set(all_bbh['SEED']) 
        indice_dco_all=0
        for index,sample in enumerate(batch['samples']):
            if sample.properties['SEED'] in all_bbh_seeds:
                sample.properties['final_mass_1'] = final_masses_1[indice_dco_all]
                sample.properties['final_mass_2'] = final_masses_2[indice_dco_all]
                sample.properties['bbh'] = 1
                indice_dco_all+=1
...
def rejected_systems(locations, dimensions):
    ...
    """
    # qui rigetta i sistemi di stelle che si creano con r1 e r2 t.c. la distanza tra le stelle sia minore del semiasse maggiore
    m1 = dimensions[0]
    q = dimensions[1]
    a = dimensions[2]
    z = dimensions[3]
